...
def sentence_builder(quantity, tech_worker_type, countries, place, activity_list, morning):
    return f"""The {quantity} {tech_worker_type} from {"and ".join(countries)} went ti teh {place} where they {" and ".join(activity_list)} until the {"morning" if morning else "night"}"""
...
demo = gr.Interface(
    ...
    inputs = [
        ...
        gr.CheckboxGroup(["Canada","Japan","Nepal"], label="Countries", info="Where are they from?"),
        gr.Radio(["office", "restaurant", "meeting room"], label="Loaction", info ="Where did they go?"),
        gr.Dropdown(
        ...
        gr.Checkbox(label="Morning",info="Did they do it the morning?"),
